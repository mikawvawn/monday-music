...
// Editorial palette — hardcoded so email clients don't need CSS var support
const BG        = "#f5ede0";
...
function newsSection(releases: CuratedRelease[]): string {
  ...
  return `<tr><td style="padding:0 28px 4px;">
    <table width="100%" cellpadding="0" cellspacing="0" border="0">
      ${secHeader("02", "MUSIC NEWS", "CURATED FOR YOU")}
...
function releasesSection(releases: CuratedRelease[]): string {
  const cards = releases.slice(0, 5).map((r, i) => {
    ...
    return `<tr>
      <td style="padding:14px 0;border-bottom:${isLast ? "none" : `1px solid ${BORDER}`};">
        <table width="100%" cellpadding="0" cellspacing="0" border="0"><tr>
          <td style="font-family:'Courier New',Courier,monospace;font-size:22px;color:${DIM};line-height:1;padding-right:12px;vertical-align:top;padding-top:3px;width:30px;">${num}</td>
          <td style="vertical-align:top;">
            <table width="100%" cellpadding="0" cellspacing="0" border="0">
              <tr>
                <td style="font-family:Georgia,'Times New Roman',serif;font-size:14px;font-weight:bold;color:${TEXT};letter-spacing:.02em;line-height:1.1;">${esc(r.title)}</td>
              ...
              <tr><td colspan="2" style="font-size:11px;color:${MUTED};line-height:1.6;padding-top:6px;">${esc(r.blurb)}</td></tr>
              <tr><td colspan="2" style="padding-top:7px;">
                <a href="${esc(r.url)}" style="display:inline-block;background:${ACCENT};color:#fff;text-decoration:none;padding:3px 10px;border-radius:3px;font-family:'Courier New',Courier,monospace;font-size:8px;font-weight:bold;letter-spacing:.1em;">READ MORE →</a>
              </td></tr>
            </table>
          </td>
        </tr></table>
  ...
  return `<tr><td style="padding:0 28px 4px;">
    <table width="100%" cellpadding="0" cellspacing="0" border="0">
      ${secHeader("03", "NEW RELEASES", `${Math.min(releases.length, 5)} ALBUMS THIS WEEK`)}
...
const REASONS = [
  "TOP ARTIST", "SIMILAR TO YOUR TOP", "YOUR FAVE", "TOP ARTIST", "NEW DISCOVERY",
  "TOP ARTIST", "NEW DISCOVERY", "SIMILAR TO YOUR TOP", "TOP ARTIST", "NEW DISCOVERY",
  "TOP ARTIST", "NEW DISCOVERY", "YOUR FAVE", "TOP ARTIST", "NEW DISCOVERY",
  "TOP ARTIST", "NEW DISCOVERY", "TOP ARTIST",
];

function playlistSection(tracks: Track[], playlistUrl: string, sectionNum: string): string {
  const rows = tracks.slice(0, 18).map((t, i) => {
    const num = String(i + 1).padStart(2, "0");
    const reason = REASONS[i % REASONS.length];
    return `<tr>
      <td style="padding:7px 10px;">
        <table width="100%" cellpadding="0" cellspacing="0" border="0"><tr>
          <td style="font-family:'Courier New',Courier,monospace;font-size:17px;color:${ACCENT};line-height:1;padding-right:14px;vertical-align:middle;width:28px;">${num}</td>
          <td style="vertical-align:middle;">
            <div>
              <span style="font-size:12px;font-weight:600;color:${TEXT};">${esc(t.name)}</span>
              <span style="font-family:'Courier New',Courier,monospace;font-size:8px;color:${ACCENT};background:${ACCENT_BG};padding:1px 5px;border-radius:2px;letter-spacing:.08em;margin-left:6px;">${reason}</span>
            </div>
            <div style="font-size:10px;color:${MUTED};margin-top:1px;">${esc(t.artist)}</div>
          </td>
        </tr></table>
      </td>
    </tr>`;
  ...
  return `<tr><td style="padding:0 28px 8px;">
    <table width="100%" cellpadding="0" cellspacing="0" border="0">
      ...
      <tr><td style="padding:4px 0 2px;"><table width="100%" cellpadding="0" cellspacing="0" border="0">${rows}</table></td></tr>
...
<table width="600" cellpadding="0" cellspacing="0" border="0" align="center" style="background:${BG};box-shadow:0 8px 32px rgba(0,0,0,0.28);">
          ...
          <td style="padding:14px 0;">
            <table width="100%" cellpadding="0" cellspacing="0" border="0"><tr>
              ...
              <td style="width:52%;vertical-align:top;padding-left:14px;border-left:1px solid ${BORDER};">
                <div style="font-family:'Courier New',Courier,monospace;font-size:8px;color:${DIM};letter-spacing:.1em;margin-bottom:8px;">NEW DISCOVERIES · ${newDiscoveries.length} ARTISTS</div>
                <div>${discTags}</div>
                <div style="margin-top:16px;font-family:'Courier New',Courier,monospace;font-size:8px;color:${DIM};letter-spacing:.1em;margin-bottom:5px;">THIS WEEK'S PLAYLIST</div>
                <div style="font-family:Georgia,serif;font-size:14px;color:${TEXT};font-weight:bold;line-height:1.2;">${esc(playlistName)}</div>
                <div style="font-size:11px;color:${MUTED};margin-top:4px;line-height:1.5;">${esc(description)}</div>
              </td>
            </tr></table>
          </td>
        </tr>
  ...
  ${playlistSection(tracks, playlistUrl, hasReleases ? "04" : "02")}
